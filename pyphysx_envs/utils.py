...
def follow_tool_tip_traj(env, poses, rewards_to_track_name=('spheres'), add_end_steps=10, custom_hammer_return=False,
                         verbose=False, return_tool_pose_list=True, save_to_file_name=None):
    ...
    for i in range(len(poses) + add_end_steps):
        ...
        # from video we reconstruct tip pose, but we can control only handle
        ...
        tool_pose_list.append(multiply_transformations(env.scene.tool.get_global_pose(), env.scene.tool.to_tip_transform))
        if verbose:
            print(rewards)
        if 'is_terminal' in rewards and rewards['is_terminal']:
            ...
            if custom_hammer_return:
                ...
                return 0., 0.
        # if 'is_terminal' in rewards and rewards['is_terminal']:
        #     if 'spheres' in rewards_to_track_name and total_reward_to_track > 0:
        #         if custom_hammer_return:
        #             return total_reward_to_track, traj_follow_reward, i
        #         if return_tool_pose_list:
        #             return total_reward_to_track, traj_follow_reward, tool_pose_list[:i-1]
        #         else:
        ...
        total_reward_to_track += (sum([rewards[reward_to_track] for reward_to_track in rewards_to_track_name]) - base_reward_to_track) / (len(poses) + add_end_steps)
        # total_reward_to_track += (rewards[reward_to_track] - base_reward_to_track + rewards['box_displacement'])/ len(poses)
        total_reward_to_track_list.append(total_reward_to_track)
        # print(total_reward_to_track)
        if save_to_file_name is not None:
            pickle.dump(total_reward_to_track_list,
        ...
        if ('nail_hammered' in rewards_to_track_name or 'cutted_grass' in rewards_to_track_name) and total_reward_to_track > 0:
            ...
            if custom_hammer_return:
                ...
                if return_tool_pose_list:
                    return scale_change * total_reward_to_track, scale_change * traj_follow_reward, tool_pose_list + [tool_pose_list[-1]]*5
                else:
