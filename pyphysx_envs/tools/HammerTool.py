...
class HammerTool(RigidDynamic):
    ...
    def __init__(self, mass=0.1, head_length=0.2, head_width=0.1, head_cover_width=0.01,
        ...
        hammer_head = Shape.create_box([head_width, head_length - head_cover_width / 2, head_width], self.mat_head)
        hammer_head.set_flag(ShapeFlag.SIMULATION_SHAPE, False)
        hammer_head.set_user_data({'color': 'tab:gray', 'name': 'hammer_head'})
        hammer_head_up = Shape.create_box([0.8 * head_width, head_cover_width, 0.8 * head_width], self.mat_head)
        hammer_head_up.set_local_pose([0., head_length / 2 - head_cover_width / 2, 0.])
        hammer_head_up.set_flag(ShapeFlag.VISUALIZATION, False)
        hammer_head_bot = Shape.create_box([0.8 * head_width, head_cover_width, 0.8 * head_width], self.mat_head)
        hammer_head_bot.set_local_pose([0., -head_length / 2 + head_cover_width / 2, 0.])
        hammer_head_bot.set_flag(ShapeFlag.VISUALIZATION, False)
        ...
        hammer_handle.set_user_data({'color': 'tab:brown'})
